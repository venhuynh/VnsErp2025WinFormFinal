    ...
    public class LoginBll
    {
        ...
        public LoginResult XacThucDangNhap(string userName, string password)
        {
            try
            {
                ...
                // Tìm user theo userName - use GetAll() and filter
                var allUsers = GetDataAccess().GetAll();
                var user = allUsers.FirstOrDefault(u => u.UserName.Equals(userName.Trim(), StringComparison.OrdinalIgnoreCase));
                if (user == null)
                {
                    return new LoginResult
                    {
        ...
        /// <returns>ApplicationUserDto đã tạo</returns>
        public ApplicationUserDto TaoUserMoi(string userName, string password, bool active = true)
        {
            try
            {
                ...
                {
                    Id = Guid.Empty, // Will be generated by Create method
                    UserName = userName.Trim(),
                    HashPassword = hashedPassword,
                    Active = active
                };
                
                // Create expects ApplicationUserDto
                return GetDataAccess().Create(userDto);
            }
            catch (Exception)
            {
                throw;
