...
public partial class FrmXuatLuuChuyenKho : DevExpress.XtraEditors.XtraForm
{
    ...
    private async void ReloadDataSourceBarButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
    {
        try
        {
            ...
            try
            {
                ...
                await Task.WhenAll(
                    ucXuatChuyenKhoMaster1.LoadLookupDataAsync(),
                    ucXuatLuuChuyenKhoDetail1.ReloadProductVariantDataSourceAsync()
                );
                ...
                using (var frmGetIdentifier = new FrmGetIdentifierForStockOut())
                {
                    ...
                    if (frmGetIdentifier.ShowDialog(this) == DialogResult.OK)
                    {
                        ...
                        if (newDetails != null && newDetails.Count > 0)
                        {
                            ...
                            await ucXuatLuuChuyenKhoDetail1.AddOrMergeDetailsAsync(newDetails);
    ...
    /// <summary>
    /// Event handler cho nút Nhập quản lý tài sản (deprecated - không còn trong bar2)
    /// </summary>
    ...
    private async Task<bool> SaveDataAsync()
    {
        try
        {
            ...
            var detailDtos = ucXuatLuuChuyenKhoDetail1.GetDetails();
