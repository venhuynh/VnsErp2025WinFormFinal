...
public partial class UcXuatLuuChuyenKhoDetail : DevExpress.XtraEditors.XtraUserControl
{
    ...
    public UcXuatLuuChuyenKhoDetail()
    {
        InitializeComponent();
        InitializeControl();
    }
    ...
    private void InitializeControl()
    {
        try
        {
            ...
            xuatLuuChuyenKhoDetailDtoBindingSource.DataSource = new List<XuatLuuChuyenKhoDetailDto>();
    ...
    private void InitializeEvents()
    {
        ...
        XuatLuuChuyenKhoDetailDtoGridView.CellValueChanged += StockInDetailDtoGridView_CellValueChanged;

        // Event khi thêm/xóa dòng để cập nhật LineNumber
        XuatLuuChuyenKhoDetailDtoGridView.InitNewRow += StockInDetailDtoGridView_InitNewRow;
        XuatLuuChuyenKhoDetailDtoGridView.RowDeleted += StockInDetailDtoGridView_RowDeleted;

        // Event khi validate cell và row
        XuatLuuChuyenKhoDetailDtoGridView.ValidateRow += StockInDetailDtoGridView_ValidateRow;
        XuatLuuChuyenKhoDetailDtoGridView.ValidatingEditor += StockInDetailDtoGridView_ValidatingEditor;

        // Event custom draw row indicator
        XuatLuuChuyenKhoDetailDtoGridView.CustomDrawRowIndicator += StockInDetailDtoGridView_CustomDrawRowIndicator;

        // Event xử lý phím tắt cho GridView (Insert, Delete, Enter)
        XuatLuuChuyenKhoDetailDtoGridView.KeyDown += StockInDetailDtoGridView_KeyDown;
    ...
    public List<StockInOutDetail> GetDetails()
    {
        try
        {
            ...
            foreach (var item in xuatLuuChuyenKhoDetailDtoBindingSource)
            {
                if (item is not XuatLuuChuyenKhoDetailDto detailDto) continue;
                ...
                details.Add(new StockInOutDetail
                {
                    ...
                    StockInQty = 0, // XuatLuuChuyenKho chỉ có xuất, không có nhập
    ...
    public void ClearData()
    {
        ...
        {
            xuatLuuChuyenKhoDetailDtoBindingSource.DataSource = new List<XuatLuuChuyenKhoDetailDto>();
            xuatLuuChuyenKhoDetailDtoBindingSource.ResetBindings(false);
    ...
    public Task LoadDataAsyncForEdit(Guid stockInOutMasterId)
    {
        try
        {
            ...
            // Convert detail entities sang DTOs sử dụng extension method từ XuatLuuChuyenKho namespace
            var detailDtos = detailEntities
                .Where(e => e != null)
                .Select(entity => entity.ToXuatLuuChuyenKhoDetailDto()) // Extension method từ XuatLuuChuyenKho namespace
                .Where(dto => dto != null)
                .ToList();
    ...
    public void SetStockOutMasterId(Guid stockOutMasterId)
    {
        try
        {
            ...
            List<XuatLuuChuyenKhoDetailDto> details;
            if (xuatLuuChuyenKhoDetailDtoBindingSource.DataSource is List<XuatLuuChuyenKhoDetailDto> dataSourceList)
            {
                details = dataSourceList;
            }
            else
            {
                details = xuatLuuChuyenKhoDetailDtoBindingSource.OfType<XuatLuuChuyenKhoDetailDto>().ToList();
            }
    ...
    private void StockInDetailDtoGridView_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
    {
        try
        {
            ...
            if (XuatLuuChuyenKhoDetailDtoGridView.GetRow(rowHandle) is not XuatLuuChuyenKhoDetailDto rowData)
            {
            ...
            XuatLuuChuyenKhoDetailDtoGridView.RefreshRow(rowHandle);
        }
        catch (Exception ex)
        {
    ...
    private void StockInDetailDtoGridView_InitNewRow(object sender, DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs e)
    {
        try
        {
            var rowData = XuatLuuChuyenKhoDetailDtoGridView.GetRow(e.RowHandle) as XuatLuuChuyenKhoDetailDto;
            if (rowData == null)
            {
    ...
    private void StockInDetailDtoGridView_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
    {
        try
        {
            var rowData = e.Row as XuatLuuChuyenKhoDetailDto;
            if (rowData == null)
            {
    ...
    private void StockInDetailDtoGridView_KeyDown(object sender, KeyEventArgs e)
    {
        try
        {
            var gridView = XuatLuuChuyenKhoDetailDtoGridView;
    ...
    /// </summary>
    /// <param name="details">Danh sách XuatLuuChuyenKhoDetailDto chứa ProductVariantId</param>
    private async Task LoadProductVariantsByIdsAsync(List<XuatLuuChuyenKhoDetailDto> details)
    {
        try
        {
    ...
    /// </summary>
    private async void LoadDetails(List<XuatLuuChuyenKhoDetailDto> details)
    {
        try
        {
            details ??= new List<XuatLuuChuyenKhoDetailDto>();
            ...
            xuatLuuChuyenKhoDetailDtoBindingSource.DataSource = details;
            xuatLuuChuyenKhoDetailDtoBindingSource.ResetBindings(false);
    ...
    private void HandleProductVariantIdChange(DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
    {
        ...
        if (rowHandle < 0)
        {
            ...
            XuatLuuChuyenKhoDetailDtoGridView.SetFocusedRowCellValue("ProductVariantName", $"{selectedVariant.VariantFullNameHtml}");
        ...
        else
        {
            ...
            if (XuatLuuChuyenKhoDetailDtoGridView.GetRow(rowHandle) is XuatLuuChuyenKhoDetailDto rowData)
            {
                rowData.ProductVariantId = selectedVariant.Id;
                rowData.ProductVariantCode = selectedVariant.VariantCode;
    ...
    /// <summary>
    /// Map XuatLuuChuyenKhoDetailDto sang StockInOutDetail entity
    /// </summary>
    private StockInOutDetail MapDetailDtoToEntity(XuatLuuChuyenKhoDetailDto dto)
    {
        ...
        return new StockInOutDetail
        {
            ...
            UnitPrice = 0, // XuatLuuChuyenKho không có giá
            Vat = 0, // XuatLuuChuyenKho không có VAT
            VatAmount = 0, // XuatLuuChuyenKho không có VAT
            TotalAmount = 0, // XuatLuuChuyenKho không có tổng tiền
            TotalAmountIncludedVat = 0, // XuatLuuChuyenKho không có tổng tiền
