...
{
    public partial class FrmXuatLapRap : DevExpress.XtraEditors.XtraForm
    {
        ...
        /// <summary>
        /// ID phiếu xuất linh kiện lắp ráp hiện tại (nếu đang edit)
        /// </summary>
        ...
        /// </summary>
        public FrmXuatLapRap()
        {
            InitializeComponent();
            Load += FrmXuatLapRap_Load;
            _currentStockOutId = Guid.Empty;
        }

        /// <summary>
        /// Constructor với ID phiếu xuất linh kiện lắp ráp (mở để xem/sửa)
        /// </summary>
        /// <param name="stockOutId">ID phiếu xuất linh kiện lắp ráp</param>
        public FrmXuatLapRap(Guid stockOutId)
        {
            InitializeComponent();
            Load += FrmXuatLapRap_Load;

            // Gán ID phiếu xuất linh kiện lắp ráp hiện tại
        ...
        /// </summary>
        private async void FrmXuatLapRap_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                await ucXuatLapRapMasterDto1.LoadLookupDataAsync();
                ...
                if (_currentStockOutId != Guid.Empty)
                {
                    ...
                    await ucXuatLapRapMasterDto1.LoadDataAsync(_currentStockOutId);
                    await ucXuatLapRapDetailDto1.LoadDataAsyncForEdit(_currentStockOutId);
                }
                else
                {
            ...
            {
                _logger.Error("FrmXuatLapRap_Load: Exception occurred", ex);
        ...
        private void SetupEvents()
        {
            try
            {
                ...
                // Form events
                FormClosing += FrmXuatLapRap_FormClosing;
                KeyDown += FrmXuatLapRap_KeyDown;
                ...
                ucXuatLapRapDetailDto1.DetailDataChanged += UcXuatLapRapDetailDto1_DetailDataChanged;
    ...
    /// </summary>
        private async void LuuPhieuBarButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!success) return;
                    MsgBox.ShowSuccess("Lưu phiếu xuất linh kiện lắp ráp thành công!", "Thành công", this);
                    MarkAsSaved();
                }
                finally
                {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (_hasUnsavedChanges)
                    {
                        ...
                        if (MsgBox.ShowYesNo(
                                "Phiếu xuất linh kiện lắp ráp chưa được lưu. Bạn có muốn lưu trước khi in không?",
                                "Xác nhận",
                                this))
                        {
                    ...
                    else
                    {
                        ...
                        MsgBox.ShowWarning(
                            "Vui lòng nhập và lưu phiếu xuất linh kiện lắp ráp trước khi in.",
                            "Cảnh báo",
                            this);
                        _logger.Warning(
                ...
                {
                    MsgBox.ShowWarning(
                        "Không thể lấy ID phiếu xuất linh kiện lắp ráp. Vui lòng thử lại.",
                        "Cảnh báo",
                        this);
                    _logger.Warning(
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (_hasUnsavedChanges)
                    {
                        // Hỏi người dùng có muốn lưu trước không
                        if (MsgBox.ShowYesNo(
                                "Phiếu xuất linh kiện lắp ráp chưa được lưu. Bạn có muốn lưu trước khi xuất quản lý tài sản không?",
                                "Xác nhận",
                                this))
                        {
                            // Gọi nút Lưu để lưu phiếu
                    ...
                    else
                    {
                        // Không có thay đổi chưa lưu và chưa có ID - yêu cầu lưu
                        MsgBox.ShowError(
                            "Vui lòng nhập và lưu phiếu xuất linh kiện lắp ráp trước khi xuất quản lý tài sản.",
                            "Lỗi",
                            this);
                        _logger.Warning(
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (_hasUnsavedChanges)
                    {
                        // Hỏi người dùng có muốn lưu trước không
                        if (MsgBox.ShowYesNo(
                                "Phiếu xuất linh kiện lắp ráp chưa được lưu. Bạn có muốn lưu trước khi thêm hình ảnh không?",
                                "Xác nhận",
                                this))
                        {
                            // Gọi nút Lưu để lưu phiếu
                    ...
                    else
                    {
                        // Không có thay đổi chưa lưu và chưa có ID - yêu cầu lưu
                        MsgBox.ShowError(
                            "Vui lòng nhập và lưu phiếu xuất linh kiện lắp ráp trước khi thêm hình ảnh.",
                            "Lỗi",
                            this);
                        _logger.Warning(
        ...
        /// </summary>
        private void FrmXuatLapRap_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
            ...
            {
                _logger.Error("FrmXuatLapRap_KeyDown: Exception occurred", ex);
        ...
        /// </summary>
        private void UcXuatLapRapDetailDto1_DetailDataChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var details = ucXuatLapRapDetailDto1.GetDetails();
                var totalQuantity = details.Sum(d => d.StockOutQty);

                // Cập nhật tổng lên master (chỉ có totalQuantity)
                ucXuatLapRapMasterDto1.UpdateTotals(totalQuantity);
            }
            catch (Exception ex)
            {
                _logger.Error("UcXuatLapRapDetailDto1_DetailDataChanged: Exception occurred", ex);
        ...
        /// </summary>
        private async void FrmXuatLapRap_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                        ...
                        case DialogResult.Yes:
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    _logger.Warning(
                                        "FrmXuatLapRap_FormClosing: Save failed, form will remain open");
                                    e.Cancel = true;
                                }
                            }
                            catch (Exception saveEx)
                            {
                                _logger.Error("FrmXuatLapRap_FormClosing: Exception during save operation",
                                    saveEx);
            ...
            {
                _logger.Error("FrmXuatLapRap_FormClosing: Exception occurred", ex);
        ...
        private void ResetForm()
        {
            try
            {
                ...
                ucXuatLapRapMasterDto1.ClearData();

                // Clear detail control
                ucXuatLapRapDetailDto1.ClearData();

                // Reset tổng về 0
                ucXuatLapRapMasterDto1.UpdateTotals(0);
        ...
        private async Task<bool> SaveDataAsync()
        {
            try
            {
                ...
                var masterDto = ucXuatLapRapMasterDto1.GetDto();
                if (masterDto == null)
                {
                    _logger.Warning("SaveDataAsync: Master validation failed - GetDto() returned null");
                    MsgBox.ShowWarning("Vui lòng kiểm tra và điền đầy đủ thông tin phiếu xuất linh kiện lắp ráp", "Cảnh báo", this);
                    return false;
                }
                ...
                if (!ucXuatLapRapDetailDto1.ValidateAll())
                {
                ...
                var detailEntities = ucXuatLapRapDetailDto1.GetDetails();
                ...
                for (var i = 0; i < detailEntities.Count; i++)
                {
                    ...
                    {
                        validationErrors.Add($"Dòng {lineNumber}: Vui lòng chọn linh kiện");
                    }

                    if (detail.StockOutQty <= 0)
                    {
                        validationErrors.Add($"Dòng {lineNumber}: Số lượng xuất linh kiện phải lớn hơn 0");
                    }
                }
                ...
                ucXuatLapRapDetailDto1.SetStockOutMasterId(savedMasterId);
                return true;
            }
