...
public partial class FrmNhapLuuChuyenKho : DevExpress.XtraEditors.XtraForm
{
    ...
    public FrmNhapLuuChuyenKho(Guid stockInId)
    {
        InitializeComponent();
        Load += FrmNhapNoiBo_Load;
    ...
    private async void FrmNhapNoiBo_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (_currentStockInId != Guid.Empty)
            {
                ...
                await ucNhapLuuChuyenKhoMaster1.LoadDataAsync(_currentStockInId);
                await ucNhapLuuChuyenKhoDetail1.LoadDataAsyncForEdit(_currentStockInId);
            }
            else
            {
