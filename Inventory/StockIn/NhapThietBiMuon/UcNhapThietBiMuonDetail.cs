...
public partial class UcNhapThietBiMuonDetail : DevExpress.XtraEditors.XtraUserControl
{
    ...
    private void StockInDetailDtoGridView_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
    {
        try
        {
            ...
            if (NhapThietBiMuonDetailDtoGridView.GetRow(rowHandle) is not StockInOutDetailForUIDto rowData)
            {
    ...
    private void StockInDetailDtoGridView_InitNewRow(object sender, DevExpress.XtraGrid.Views.Grid.InitNewRowEventArgs e)
    {
        try
        {
            var rowData = NhapThietBiMuonDetailDtoGridView.GetRow(e.RowHandle) as StockInOutDetailForUIDto;
            if (rowData == null)
            {
    ...
    private void HandleProductVariantIdChange(DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
    {
        ...
        if (rowHandle < 0)
        {
            ...
            NhapThietBiMuonDetailDtoGridView.SetFocusedRowCellValue("ProductVariantName", $"{selectedVariant.VariantFullName}");
        ...
        else
        {
            ...
            if (NhapThietBiMuonDetailDtoGridView.GetRow(rowHandle) is StockInOutDetailForUIDto rowData)
            {
                rowData.ProductVariantId = selectedVariant.Id;
                rowData.ProductVariantCode = selectedVariant.VariantCode;
